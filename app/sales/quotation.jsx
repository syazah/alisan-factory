...
const Quotation = () => {
  ...
  async function SendToAdmin() {
    try {
      ...
      const data = await res.json();
      if (data.success === true) {
        Alert.alert("Success", "Order with quotation sent to admin");
      ...
      return Alert.alert(
        "Error",
        "Something Went wrong while sending the order to admin"
      );
    }
  }
  if (finalCosts === null) {
        ...
        <View className="flex-row">
          ...
          <TouchableOpacity
            onPress={() => {
              return Alert.alert(
                "Send To Admin",
                "This will send the order along with your quotation to the admin, it is not reversible",
                [
                  { text: "Cancel", style: "cancel" },
                  { text: "Send", onPress: () => SendToAdmin() },
                ],
                { cancelable: true }
              );
            }}
            className="w-10 h-10 bg-red-800 rounded-full justify-center items-center ml-1"
          >
