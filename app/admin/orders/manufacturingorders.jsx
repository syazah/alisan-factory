...
const ManufacturerOrders = () => {
  const [manufacturerOrders, setmanufacturerOrders] = React.useState(null);
  async function ViewOrders() {
    try {
      const res = await fetch(`${url}/api/v1/admin/manufacturer-orders`, {
        method: "GET",
        headers: { "Content-Type": "application/json" },
      });
      const data = await res.json();
      if (data.success === true) {
        return setmanufacturerOrders(data.orders);
      } else {
  ...
  if (manufacturerOrders === null) {
    return (
      ...
      >
        ...
        <Text className="mt-6 text-white">Fetching Manufacturer Orders</Text>
      </SafeAreaView>
    );
  }
  return (
    <SafeAreaView style={{ flex: 1 }} className="bg-zinc-100">
      <View className="w-full h-[10%] bg-zinc-100 justify-center items-start p-2">
        <Text className="text-3xl text-red-600 font-semibold">
          Stage-<Text className="text-black">Manufacturer</Text>
        </Text>
      </View>
        ...
        {manufacturerOrders.length === 0 ? (
          <View className="w-full h-[90%] bg-zinc-800 rounded-t-xl pt-4 justify-center items-center">
            ...
            <Text className="text-white font-normal text-base">
              No Orders Assigned To Any Manufacturer
            </Text>
          </View>
        ) : (
          <FlatList
